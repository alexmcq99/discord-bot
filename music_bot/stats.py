...
class StatsFactory:
    ...
    async def create_stats(
        ...
        *,
        ...
        ytdl_source = None
        if ytdl_args:
            ytdl_source = await anext(
                self.ytdl_source_factory.create_ytdl_sources(ytdl_args)
            )

        if user:
            self.filter_kwargs["requester_id"] = user.id
        if ytdl_source:
            self.filter_kwargs["song_id"] = ytdl_source.id
        ...
        if user and ytdl_source:
            ...
            embed_description = f"{user.mention} and {ytdl_source.link_markdown}"
            ...
            embed_description = ytdl_source.link_markdown
    ...
    def is_current_song_relevant(self) -> bool:
        return self.ctx.audio_player.is_playing and (
            "song_id" not in self.filter_kwargs
            or self.filter_kwargs["song_id"] == self.ctx.audio_player.current_song.id
        )
    ...
    async def format_request(self, request: SongRequest) -> str:
        ...
        if "song_id" not in self.filter_kwargs:
            ytdl_source = await anext(
                self.ytdl_source_factory.create_ytdl_sources(request.song_id)
            )
    ...
    async def get_most_requested_song_formatted(self) -> str:
        ...
        formatted = f"{ytdl_source.link_markdown} with {request_count} request{'s' if request_count > 1 else ''}"
        return formatted
